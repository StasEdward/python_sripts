...
if ceragon_connected:
        ...
        SELECT = "SELECT count(*) as errorresult from (select distinct serialnumber FROM phostestglobaltest WHERE testdate >= dateadd (-1 day to current_date) AND GLOBALRESULT='Error')"
        cur.execute(SELECT)
        for (errorresult) in cur:
                ceragon_error = errorresult[0]

        print 'Ceragon:\t%d Pass, %d Fail, %d Error' % (ceragon_pass,ceragon_fail, ceragon_error)
	con_ceragon.close()
else:
...
current_date) AND GLOBALRESULT='Fail')"
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                ionics1_error = errorresult[0]

        print 'Ionics1:\t%d Pass, %d Fail, %d Error' % (ionics1_pass,ionics1_fail, ionics1_error)
        con_ionics1.close()
else:
...
current_date) AND GLOBALRESULT='Fail')"
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                ionics2_error = errorresult[0]

        print 'Ionics2:\t%d Pass, %d Fail, %d Error' % (ionics2_pass,ionics2_fail, ionics2_error)
        con_ionics2.close()
else:
...
current_date) AND GLOBALRESULT='Fail')"
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                flex1_error = errorresult[0]

        print 'Flex1:\t%d Pass, %d Fail, %d Error' % (flex1_pass,flex1_fail, flex1_error)
        con_flex1.close()
else:
...
current_date) AND GLOBALRESULT='Fail')"
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                flex2_error = errorresult[0]

        print 'Flex2:\t%d Pass, %d Fail, %d Error' % (flex2_pass,flex2_fail, flex2_error)
        con_flex2.close()
else:
...
if vcl_connected:
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                vcl_error = errorresult[0]

        print 'VCL:\t%d Pass, %d Fail, %d Error' % (vcl_pass,vcl_fail, vcl_error)
        con_vcl.close()
else:
...
if vcl_connected:
        ...
        cur.execute(SELECT)
        for (errorresult) in cur:
                jbl_error = errorresult[0]

        print 'JBL1:\t%d Pass, %d Fail, %d Error' % (jbl_pass,jbl_fail, jbl_error)
        con_vcl.close()
else:
        print "VCL DB skipped"
...
DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'Ceragon\'' % (ceragon_pass, ceragon_fail, ceragon_error)
#print(DB_UPDATE)
...
flex1_error = flex1_error + flex2_error 

DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'Flex\'' % (flex1_pass, flex1_fail, flex1_error)
#print(DB_UPDATE)
cur_local = cnx.cursor()
cur_local.execute(DB_UPDATE)
cnx.commit()
...
#DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'Flex2\'' % (flex2_pass, flex2_fail, flex2_error)
#print(DB_UPDATE)
#cur_local = cnx.cursor()
#cur_local.execute(DB_UPDATE)
#cnx.commit()
...
ionics1_error = ionics1_error + ionics2_error


DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'Ionics\'' % (ionics1_pass, ionics1_fail, ionics1_error)
#print(DB_UPDATE)
cur_local = cnx.cursor()
cur_local.execute(DB_UPDATE)
cnx.commit()
...
#DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'Ionics2\'' % (ionics2_pass, ionics2_fail, ionics2_error)
#print(DB_UPDATE)
#cur_local = cnx.cursor()
#cur_local.execute(DB_UPDATE)
#cnx.commit()
...
DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'VCL\'' % (vcl_pass, vcl_fail, vcl_error)
#print(DB_UPDATE)
cur_local = cnx.cursor()
cur_local.execute(DB_UPDATE)
cnx.commit()
...
DB_UPDATE = 'UPDATE atestat.PASSFAIL_RESULTS set PASS_RESULT=%d, FAIL_RESULT=%d, ERROR_RESULT=%d, TIME_UPDATE=current_timestamp  WHERE SERVER_NAME=\'JBL\'' % (jbl_pass, jbl_fail, jbl_error)
#print(DB_UPDATE)
cur_local = cnx.cursor()
cur_local.execute(DB_UPDATE)
cnx.commit()
